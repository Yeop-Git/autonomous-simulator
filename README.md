...
[시스템](#1-시스템-구성) · [통신](#2-통신-및-데이터-규약) · [기술](#3-기술-명세) ·
[씬](#4-씬-구성) · [결과](#5-실험-결과) · [검증](#6-검증-및-제한사항) ·
[실행](#7-실행-방법) · [구성](#8-저장소-구성) · [참고문헌](#9-참고문헌)
...
아래 GIF는 Python 서버에 연결한 Unity Game View 캡처를 바탕으로 만들었다.
모바일 재생을 위해 변화가 완만한 주행 영역만 보간했으며, 성능 수치는 §5의
원자료를 기준으로 한다.
...
IntegratedCity의 A\* 전역 경로, RRT 국부 회피, 교차로·순환 대로·동적 이벤트
통합 주행은 문서 상단 대표 영상에서 확인할 수 있다.
...
재실행 명령은 [§7.3 실험 재생성](#73-실험-재생성)에 정리했다.
